...
func TestMatchTree_InsertAndMatch(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			if cat.Template != test.expect {
				t.Errorf("Expected template %q, got %q", test.expect, cat.Template)
			}
		})
	}
}
...
func TestMatchTree_WildcardPriority(t *testing.T) {
	...
	if cat.Template != "Hello there!" {
		t.Errorf("Expected 'Hello there!', got %q", cat.Template)
	}
	...
	if cat.Template != "Wildcard after hello" {
		t.Errorf("Expected 'Wildcard after hello', got %q", cat.Template)
	}
}
...
func TestMatchTree_MatchWithMeta_MetadataAndWildcards(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			res, found := tree.MatchWithMeta(test.input, test.that, test.topic)
			if !found {
				t.Fatalf("No match found for input: %q, that: %q, topic: %q", test.input, test.that, test.topic)
			}
			if res.Template != test.expectTemplate {
				t.Errorf("Expected template %q, got %q", test.expectTemplate, res.Template)
			}
			if res.MatchedPattern != test.expectPattern {
				t.Errorf("Expected matched pattern %q, got %q", test.expectPattern, res.MatchedPattern)
			}
			if res.MatchedThat != test.expectThat {
				t.Errorf("Expected matched that %q, got %q", test.expectThat, res.MatchedThat)
			}
